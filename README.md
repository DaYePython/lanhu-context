...
2. Run `npx skills add DaYePython/lanhu-context --global` to install the skill globally; if that doesn't apply to you, install the lanhu-context-cli skill into your own global skill/rule directory (e.g. Claude Code ~/.claude/skills, Codex ~/.codex/AGENTS.md);
...
3. Run `npx skills add DaYePython/lanhu-context` to install the skill (project scope is the default); if that doesn't apply to you, install the lanhu-context-cli skill into this project's skill/rule directory (e.g. .claude/skills, .cursor/rules) and note in the skill usage that commands need the `npx lanhu` prefix;
...
To teach your agent how to drive the CLI, install the bundled skills with [find-skills](https://github.com/vercel-labs/skills) (project-scoped by default; add `--global` for a global install):
