...
- `packages/mcp` —— `@lanhu-context/mcp`：MCP 兼容层，对外契约必须与上游 `lanhu-context-mcp` 的 `get_design_context` 保持一致。
...
- **stdout 纪律**：CLI 命令 stdout 只放数据（`--json` 时是统一 envelope），日志/进度一律走 stderr；`lanhu mcp --stdio` 模式 stdout 只承载 JSON-RPC 帧。
