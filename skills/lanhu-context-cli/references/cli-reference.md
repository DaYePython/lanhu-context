...
| `lanhu auth set\|status\|test\|listen` | 报告 | 凭据写入（0600）/ 状态（来源 + 掩码指纹）/ 验证 token 是否有效 / 一次性接收浏览器扩展或油猴脚本发来的 Cookie 并写入用户级配置 | 状态 envelope，永不含 token 明文（listen 的监听提示走 stderr） |
...
| `auth listen` | `--port <port>` | `7623` | 监听端口（1-65535，非法值 exit 2）；需与 `ecosystem/ecosystem-core/src/constants.ts` 的 `DEFAULT_BRIDGE_PORT` 一致（扩展与油猴脚本共享该常量） |
...
`auth listen`：配合仓库 `ecosystem/browser-extension` 浏览器扩展或 `ecosystem/lanhu-monkey` 油猴脚本使用——终端启动后在蓝湖设计稿页面右键点「发送 cookies 到本机」。安全模型：只接受 `Origin: chrome-extension://`（扩展；网页无法伪造该头）或携带 `x-lanhu-bridge` 请求头（油猴脚本；网页无法免 CORS preflight 携带自定义头）的请求，非法来源一律 403 且不写盘 + 仅监听 127.0.0.1 + 收到一次即退出 + 超时自动退出。成功输出（实测，FAKE 演示 token，路径缩略）：
