...
//
// On engine failure the original HTML is returned unchanged (degraded, not
// fatal): callers can observe the fallback via the `onFallback` option.
...
export interface ConvertHtmlToTailwindOptions {
  ...
  twVersion?: 3 | 4;
  // Called when conversion fails and the original HTML is returned as-is.
  // Defaults to logging the failure on console.error.
...
export async function convertHtmlToTailwind(
  ...
  if (customRules.length > 0) {
    try {
      ...
      // Fall back to the original HTML and keep the failure visible.
      if (options.onFallback) {
        options.onFallback(err);
      } else {
        console.error(
          `[css-to-tailwind] v${twVersion} conversion failed:`,
          err
        );
      }
      return html;
