...
// `assetsDir` overrides the default `./src/assets/<design-name>` local
// path prefix (a trailing slash is normalized away).
export function localizeImageUrls(
  ...
  const dirPrefix = assetsDir?.trim()
    ? assetsDir.trim().replace(/\/+$/, '')
    : `./src/assets/${safeName}`;
    ...
    (match, prefix: string, url: string, suffix: string) => {
      if (
        ...
        const ext = /^(png|jpg|jpeg|gif|svg|webp|bmp|ico)$/i.test(rawExt)
          ? rawExt.toLowerCase()
          : 'png';
        const localPath = `${dirPrefix}/icon-${counter}.${ext}`;
