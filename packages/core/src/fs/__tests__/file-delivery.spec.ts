// Unit tests for file delivery utilities, including the content-hash
// idempotency model (written / skipped / overwritten + force).
import { Buffer } from 'node:buffer';
import { mkdtemp, readFile, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { resolve as resolvePath } from 'node:path';
import { afterEach, beforeEach } from 'vitest';
import { sanitizeDesignDirName, writeDesignFiles } from '../file-delivery';
...
describe('writeDesignFiles', () => {
  ...
  test('writes context.md verbatim and returns FileInfo with status "written"', async () => {
    ...
    expect(result.files.context.status).toBe('written');
  ...
  test('writes preview.png when buffer provided', async () => {
    ...
    expect(result.files.preview!.status).toBe('written');
...
describe('writeDesignFiles — content-hash idempotency (DESIGN.md §5.2)', () => {
  let tmpRoot: string;
  ...
  beforeEach(async () => {
    tmpRoot = await mkdtemp(resolvePath(tmpdir(), 'lanhu-idem-'));
  });
  ...
  test('re-running with identical content reports "skipped"', async () => {
    const buf = Buffer.from([1, 2, 3]);
    await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'same body',
      previewBuffer: buf
    });
    const second = await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'same body',
      previewBuffer: buf
    });
    expect(second.files.context.status).toBe('skipped');
    expect(second.files.preview!.status).toBe('skipped');
  });

  test('re-running with different content reports "overwritten" and updates the file', async () => {
    await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'old body'
    });
    const second = await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'new body'
    });
    expect(second.files.context.status).toBe('overwritten');
    const written = await readFile(second.files.context.path, 'utf8');
    expect(written).toBe('new body');
  });

  test('same size but different content is NOT skipped (hash comparison, not size)', async () => {
    await writeDesignFiles({ ...base, outDir: tmpRoot, contextBody: 'aaaa' });
    const second = await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'bbbb'
    ...
    expect(second.files.context.status).toBe('overwritten');
    expect(await readFile(second.files.context.path, 'utf8')).toBe('bbbb');
  });

  test('force: true rewrites even identical content and reports "overwritten"', async () => {
    await writeDesignFiles({ ...base, outDir: tmpRoot, contextBody: 'body' });
    const second = await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'body',
  ...
  test('force: true on a fresh path still reports "written"', async () => {
    const result = await writeDesignFiles({
      ...base,
      outDir: tmpRoot,
      contextBody: 'body',
